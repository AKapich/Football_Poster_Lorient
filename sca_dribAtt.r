...
minute_remark <- paste("Only players who played over",
                       time_threshold, "90s included.")

Lorient <- dataset %>% 
  filter(Player %in% c("Terem Moffi", "Julien Ponceau"))

plot <- ggplot(dataset, aes(x=.data$SCA90, y=.data$dribAtt90)) +
  #ggtitle("Ligue 1 2022/2023: SCA vs DribAtt") +
  geom_point() +
  xlab("Shot Creating Actions per 90 minutes") +
  ylab("Dribbles Attempted per 90 minutes") +
  ...
  geom_text_repel(data=Lorient,
                  aes(.data$SCA90,.data$dribAtt90,label=.data[["Player"]]),
                  size=4, colour="gray20") +
  labs(caption=paste(minute_remark,
                     "Stats by Opta via FBref."))
plot(plot)

ggsave(filename = "CANVA_SCAvsDribAtt.png",
       plot=plot,
       bg='transparent',
       width=11.7,
       height=6)