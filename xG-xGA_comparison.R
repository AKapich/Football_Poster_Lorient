...
L1_22_stats <- fb_match_summary(match_url=L1_22_urls)
...
# usunięcie ostatniego meczu żeby nie psuć kodu:
L1_22_stats <- head(L1_22_stats, 12)
...
ggplot(data = team_xGdiffxGA, aes(x = gameweek, y = -4)) +
  #ggtitle("Lorient-Expected Goals Comparison 22/23") +
  geom_point() +
  xlab("Gameweek") +
  ylab("xG Against v xG") +
  theme_classic()+
  ...
  coord_cartesian(ylim = c(-4, 4)) + scale_x_continuous(breaks = 1:as.numeric(last_gw$gameweek)) + scale_y_continuous(breaks = c(as.numeric(round(xga_mean$xga_mean,2)),as.numeric(round(xg_mean$xg_mean,2))))
